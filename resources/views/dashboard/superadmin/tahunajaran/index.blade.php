                ...
                <table class="table align-middle mb-0 table-hover">
                    ...
                    <tbody style="font-size: 14px;">
                        @forelse($tahunAjarans as $ta)
                            <tr>
                                <td class="ps-4">
                                    <div class="fw-800 {{ $ta->is_aktif ? 'text-dark' : 'text-muted' }}">{{ $ta->nama }}
                                    </div>
                                ...
                                <td class="text-end pe-4">
                                    ...
                                    {{-- Form Hapus dengan ID Dinamis --}}
                                    <form id="delete-form-{{ $ta->id }}"
                                        action="{{ route('superadmin.tahun-ajaran.destroy', $ta->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-light btn-sm text-danger rounded-3"
                                            onclick="confirmDelete('delete-form-{{ $ta->id }}')">
                            ...
                            <div class="modal fade" id="modalEdit{{ $ta->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                                        ...
                                        <form action="{{ route('superadmin.tahun-ajaran.update', $ta->id) }}"
                                            method="POST">
                                            @csrf @method('PUT')
                                            <div class="modal-body p-4">
                                                <div class="row g-3 mb-3">
                                                    <div class="col-6">
                                                        <label class="form-label small fw-bold text-muted">TAHUN
                                                            MULAI</label>
                                                        <input type="number" name="tahun_mulai" class="form-control"
                                                            value="{{ $ta->tahun_mulai }}" required>
                                                    </div>
                                                    <div class="col-6">
                                                        <label class="form-label small fw-bold text-muted">TAHUN
                                                            SELESAI</label>
                                                        <input type="number" name="tahun_selesai" class="form-control"
                                                            value="{{ $ta->tahun_selesai }}" required>
                                                    </div>
                                                </div>
                                                <div class="mb-4">
                                                    <label class="form-label small fw-bold text-muted">STATUS GLOBAL</label>
                                                    <select name="is_aktif" class="form-select">
                                                        <option value="1" {{ $ta->is_aktif ? 'selected' : '' }}>Aktif
                                                        </option>
                                                        <option value="0" {{ !$ta->is_aktif ? 'selected' : '' }}>
                                                            Non-Aktif</option>
                                                    </select>
                                                </div>
                                                <div class="d-grid">
                                                    <button type="submit"
                                                        class="btn btn-primary fw-800 py-3 rounded-4">Simpan
                                                        Perubahan</button>
                                                </div>
                                            </div>
                                        </form>
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-6">
                                ...
                                <input type="number" name="tahun_selesai" class="form-control" placeholder="2026"
                                    required>
                            </div>
                        </div>
                        <div class="mb-4">
                            ...
                            <div class="d-flex gap-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="is_aktif" value="1"
                                        id="addAktif">
                                    <label class="form-check-label" for="addAktif">Aktif</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="is_aktif" value="0"
                                        id="addNon" checked>
                        ...
                        <div class="d-grid">
                            <button type="submit" class="btn btn-success fw-800 py-3 rounded-4 shadow">Simpan
                                Periode</button>
                        </div>
                    </div>
                </form>
