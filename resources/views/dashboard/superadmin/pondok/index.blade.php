                ...
                <tbody style="font-size: 14px;">
                    ...
                    <tr>
                        <td class="ps-3">
                            ...
                            <div style="font-size: 11px;" class="text-muted text-uppercase fw-bold">ID: {{ $p->kode_pondok }}</div>
                        </td>
                        <td><div class="small fw-bold">{{ $p->pengasuh ?? '-' }}</div></td>
                        <td>
                            @if($p->yayasan_mitra == 'Yayasan')
                                <span class="badge bg-success-subtle text-success border border-success-subtle px-3 py-2 rounded-pill">Milik Yayasan</span>
                            @else
                                <span class="badge bg-warning-subtle text-warning border border-warning-subtle px-3 py-2 rounded-pill">Mitra / Luar</span>
                            @endif
                        </td>
                        <td>
                            <form action="{{ route('superadmin.pondok.toggle', $p->id) }}" method="POST">
                                ...
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" onchange="this.form.submit()" {{ $p->is_aktif ? 'checked' : '' }}>
                                </div>
                            </form>
                        </td>
                        <td class="text-end pe-3">
                            <button class="btn btn-light btn-sm text-primary me-1" onclick='editPondok(@json($p))' data-bs-toggle="modal" data-bs-target="#modalPondok">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form action="{{ route('superadmin.pondok.destroy', $p->id) }}" method="POST" class="d-inline">
                                @csrf @method('DELETE')
                                <button class="btn btn-light btn-sm text-danger" onclick="return confirm('Hapus data ini?')">
                ...
                <div class="modal-body p-4">
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="kode_pondok" id="kode_pondok" class="form-control" placeholder="P01" required>
                        </div>
                        <div class="col-md-8">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex gap-2">
                            <input type="radio" class="btn-check" name="jenis" id="jenisL" value="L" checked>
                            <label class="btn btn-outline-primary flex-grow-1 rounded-3 py-2 fw-bold" for="jenisL">L (Putra)</label>

                            <input type="radio" class="btn-check" name="jenis" id="jenisP" value="P">
                            <label class="btn btn-outline-danger flex-grow-1 rounded-3 py-2 fw-bold" for="jenisP">P (Putri)</label>

                            <input type="radio" class="btn-check" name="jenis" id="jenisLP" value="LP">
                            <label class="btn btn-outline-success flex-grow-1 rounded-3 py-2 fw-bold" for="jenisLP">L/P (Campuran)</label>
                        </div>
                    </div>

                    <div class="mb-2"><label class="form-label small fw-bold text-muted text-uppercase">Status Kepemilikan</label></div>
                    <div class="list-group border-0">
                        <label class="list-group-item d-flex gap-3 py-3 border rounded-4 mb-2 shadow-sm">
                            <input class="form-check-input flex-shrink-0" type="radio" name="yayasan_mitra" id="milikYayasan" value="Yayasan" checked>
                            <span class="pt-1">
                        ...
                        <label class="list-group-item d-flex gap-3 py-3 border rounded-4 shadow-sm">
                            <input class="form-check-input flex-shrink-0" type="radio" name="yayasan_mitra" id="milikMitra" value="Mitra">
                            <span class="pt-1">
