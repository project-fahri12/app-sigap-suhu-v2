    ...
    <div class="container-fluid">
        ...
        <div class="row mb-4 align-items-center">
            <div class="col-md-6">
                ...
                <p class="text-muted small mb-0">Manajemen gedung asrama santri.</p>
            </div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($asramas as $item)
                            <tr>
                                <td class="ps-4 fw-bold text-muted">{{ $item->iteration  }}</td>
                                <td>
                                ...
                                <td class="text-center">
                                    <span class="badge rounded-pill {{ $item->status_asrama == 'Aktif' ? 'bg-success-subtle text-success' : 'bg-light text-muted' }} px-3 py-2">
                                        {{ $item->status_asrama ?? 'NULL' }}
                                    </span>
                                </td>
                                <td class="text-end pe-4">
                                    <div class="btn-group border rounded-pill overflow-hidden bg-white shadow-sm">
                                        <button class="btn btn-white btn-sm border-0 py-2 px-3" data-bs-toggle="modal" data-bs-target="#modalEdit{{ $item->id }}" title="Edit">
                                    ...
                                    <div class="modal-content border-0 rounded-4">
                                        <form action="{{ route('adminpondok.asrama.update', $item->id) }}" method="POST">
                                            ...
                                            <div class="modal-header border-0 pt-4 px-4">
                                                <h5 class="fw-bold mb-0">Edit Nama Asrama</h5>
                            ...
                            @empty
                            <tr><td colspan="4" class="text-center py-5 text-muted">Data asrama masih kosong.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <div class="modal-content border-0 rounded-4 shadow">
            <form action="{{ route('adminpondok.asrama.store') }}" method="POST">
                ...
                <div class="modal-body p-4 text-center">
                    <i class="fas fa-building text-success fa-3x mb-4 opacity-25"></i>
                    <div class="text-start mb-3">
                        <label class="form-label small fw-bold text-muted">NAMA GEDUNG / ASRAMA</label>
                        <input type="text" name="nama_asrama" class="form-control rounded-3 bg-light border-0 py-2 text-center fw-bold" placeholder="Misal: Al-Firdaus" required>
                    </div>
