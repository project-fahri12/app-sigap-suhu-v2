            ...
            <table class="table align-middle mb-0 table-hover">
                ...
                <tbody style="font-size: 14px;">
                    @forelse($gelombangs as $g)
                        <tr>
                            <td class="ps-4">
                                <div class="fw-800">{{ $g->nama_gelombang }}</div>
                            </td>
                            <td>{{ $g->tahunAjaran->nama ?? '-' }}</td>
                            <td>
                                <small>{{ \Carbon\Carbon::parse($g->tanggal_buka)->format('d M Y') }} - {{ \Carbon\Carbon::parse($g->tanggal_tutup)->format('d M Y') }}</small>
                            </td>
                            <td><span class="badge bg-info-subtle text-info">{{ $g->kuota }} Kursi</span></td>
                            <td>
                                <span class="badge {{ $g->is_aktif ? 'bg-success' : 'bg-light text-muted' }} px-3 py-1 rounded-pill">
                            ...
                            <td class="text-end pe-4">
                                ...
                                <form action="{{ route('adminsekolah.gelombang-ppdb.destroy', $g->id) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-light btn-sm text-danger rounded-3" onclick="return confirm('Hapus gelombang ini?')">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="tahun_ajaran_id" class="form-select" required>
                                                    @foreach($tahunAjarans as $ta)
                                                        <option value="{{ $ta->id }}" {{ $g->tahun_ajaran_id == $ta->id ? 'selected' : '' }}>{{ $ta->nama }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label small fw-bold text-muted">NAMA GELOMBANG</label>
                                                <input type="text" name="nama_gelombang" class="form-control" value="{{ $g->nama_gelombang }}" required>
                                            </div>
                                            <div class="row g-3 mb-3">
                                                <div class="col-6">
                                                    <label class="form-label small fw-bold text-muted">TGL BUKA</label>
                                                    <input type="date" name="tanggal_buka" class="form-control" value="{{ $g->tanggal_buka }}" required>
                                                </div>
                                                <div class="col-6">
                                                    <label class="form-label small fw-bold text-muted">TGL TUTUP</label>
                                                    <input type="date" name="tanggal_tutup" class="form-control" value="{{ $g->tanggal_tutup }}" required>
                                                </div>
                                            </div>
                                            <div class="row g-3 mb-3">
                                                <div class="col-6">
                                                    <label class="form-label small fw-bold text-muted">KUOTA</label>
                                                    <input type="number" name="kuota" class="form-control" value="{{ $g->kuota }}" required>
                                                </div>
                                                <div class="col-6">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">Belum ada data gelombang.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="modalTambahGelombang" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
            <div class="modal-header border-0 p-4 pb-0">
                <h5 class="fw-800 mb-0">Tambah Gelombang PPDB</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('adminsekolah.gelombang-ppdb.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">NAMA GELOMBANG</label>
                        <input type="text" name="nama_gelombang" class="form-control" placeholder="Contoh: Gelombang 1" required>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="form-label small fw-bold text-muted">TGL BUKA</label>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="form-label small fw-bold text-muted">KUOTA</label>
                            <input type="number" name="kuota" class="form-control" placeholder="100" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label small fw-bold text-muted">AKTIFKAN?</label>
                            <div class="form-check form-switch mt-2">
                                <input class="form-check-input" type="checkbox" name="is_aktif" value="1" id="flexSwitchCheckDefault">
                                <label class="form-check-label" for="flexSwitchCheckDefault">Ya, Aktifkan</label>
                            </div>
                        </div>
                    </div>
                    <div class="d-grid mt-4">
