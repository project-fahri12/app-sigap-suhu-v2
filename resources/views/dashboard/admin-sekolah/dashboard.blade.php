                    ...
                    <div class="row align-items-center position-relative" style="z-index: 2;">
                        <div class="col-md-7">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h4 class="fw-800 mb-1 letter-spacing-1">{{ auth()->user()->sekolah->nama_sekolah ?? 'Unit Sekolah' }}</h4>
                        ...
                        <div class="col-md-5 text-md-end mt-3 mt-md-0">
                            <div class="bg-white bg-opacity-10 p-3 rounded-4 d-inline-block border border-white border-opacity-10">
                                ...
                                <p class="mb-0 small opacity-75 fw-bold" id="liveDate">{{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="row g-4">
            <div class="col-xl-9">
                ...
                <div class="row g-3 mb-4">
                    @php
                        $cards = [
                            ['label' => 'PENDAFTAR', 'val' => $stats['pendaftar'], 'color' => 'primary', 'icon' => 'fa-user-plus'],
                            ['label' => 'SISWA AKTIF', 'val' => $stats['siswa_aktif'], 'color' => 'success', 'icon' => 'fa-user-check'],
                            ['label' => 'TOTAL ALUMNI', 'val' => $stats['alumni'], 'color' => 'warning', 'icon' => 'fa-user-graduate']
                        ];
                    @endphp
                    @foreach($cards as $item)
                    <div class="col-md-4">
                        <div class="card border-0 shadow-sm rounded-4 p-3 border-start border-{{ $item['color'] }} border-5 h-100 bg-white">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="bg-{{ $item['color'] }} bg-opacity-10 p-3 rounded-circle text-{{ $item['color'] }}">
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden">
                    <div class="card-header bg-white border-0 py-4 px-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <small class="text-muted">Data pendaftaran unit {{ auth()->user()->sekolah->singkatan }}</small>
                        </div>
                        <div class="search-box position-relative">
                            <i class="fas fa-search position-absolute top-50 translate-middle-y ms-3 text-muted"></i>
                            <input type="text" id="liveSearch" class="form-control border-0 bg-light rounded-pill ps-5 pe-4 py-2" placeholder="Cari nama atau kode..." autocomplete="off">
                        </div>
                    </div>
                    
                    {{-- Scrollable Container dengan ID untuk prevent jump --}}
                    <div class="table-responsive" id="tableWrapper" style="max-height: 500px; overflow-y: auto; scroll-behavior: smooth;">
                                    ...
                                    <td class="ps-4">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-muted" style="font-size: 11px;">{{ $p->kode_pendaftaran }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="badge rounded-pill fw-700 px-3 py-2 bg-{{ $p->status_pendaftaran == 'diterima' ? 'success' : 'warning' }}-subtle text-{{ $p->status_pendaftaran == 'diterima' ? 'success' : 'warning' }}">
                                            {{ ucfirst($p->status_pendaftaran) }}
                                        </span>
                                    </td>
                                    <td>
                                        @if($p->status_pendaftaran == 'diterima')
                                            <span class="text-success small fw-700"><i class="fas fa-check-circle me-1"></i> Terverifikasi</span>
                                        @else
                                            <span class="text-muted small fw-700"><i class="fas fa-clock me-1"></i> Menunggu Verifikasi</span>
                                        @endif
                                    </td>
                                    <td class="text-center pe-4">
                                        <div class="btn-group shadow-sm rounded-3">
                                            <a href="" class="btn btn-sm btn-white border px-3" title="Detail">
                                                <i class="fas fa-eye text-primary"></i>
                                            </a>
                                            <button type="button" onclick="confirmDelete(event, '{{ $p->id }}')" class="btn btn-sm btn-white border px-3" title="Hapus">
            ...
            <div class="col-xl-3">
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-white mb-4">
                    ...
                    <div class="card-body p-4">
                        <button class="btn btn-primary w-100 rounded-pill mb-3 fw-700 py-2 shadow-sm" onclick="showV2Info('Tambah Pendaftar')">
                            <i class="fas fa-plus-circle me-2"></i>Tambah Manual
                        </button>
                        <hr class="my-4 opacity-50">
                        <div class="v2-card p-3 rounded-4 border border-dashed text-center bg-light pointer" onclick="showV2Info('Laporan Semester')">
...
<style>
    ...
    .avatar-circle {
        ...
        border-radius: 50%; font-weight: 800; font-size: 13px; border: 2px solid #fff;
    }

    .custom-table thead th { border: none; font-size: 10.5px; color: #64748b; text-transform: uppercase; }
    .custom-table tbody tr { transition: background 0.2s; border-bottom: 1px solid #f1f5f9; }
    ...
    .v2-card:hover { transform: translateY(-3px); border-color: #3b82f6 !important; background: #fff !important; }
